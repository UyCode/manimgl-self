...
from manimlib.mobject.geometry import Arrow, VectorFrom
    ...
    def add_coordinate_labels(
        ...
        **kwargs
        ...
        if x_tip:
            ...
            vec_x.shift(UP * self.get_origin()[1])
            self.add(vec_x)
            kwargs.pop('x_tip')

        if y_tip:
            t = self.get_origin()[1] + float(((self.get_all_ranges()[1])[1]))
            vec_y = VectorFrom(start=np.array([0, t, 0]), end=np.array([0, t + 0.5, 0]), stroke_width=2)
            vec_y.shift(RIGHT * self.get_origin()[0])
            self.add(vec_y)
            kwargs.pop('y_tip')
